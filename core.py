...
class SQLiteCache:
    ...
    def _generate_tmdb_search_key(self, media_type: str, title: str, year: Optional[int] = None) -> str:
        """Generate a cache key for TMDb search requests."""
        import hashlib
        key_data = f"{media_type}:{title}:{year or ''}"
        return hashlib.md5(key_data.encode('utf-8')).hexdigest()

    def _generate_tmdb_details_key(self, media_type: str, media_id: int) -> str:
        """Generate a cache key for TMDb details requests."""
        import hashlib
        key_data = f"{media_type}:details:{media_id}"
        return hashlib.md5(key_data.encode('utf-8')).hexdigest()

    def get_tmdb_search_cache(self, media_type: str, title: str, year: Optional[int] = None, cache_ttl_days: int = 7) -> Optional[Dict[str, Any]]:
        """Retrieve cached TMDb search results."""
        cache_key = self._generate_tmdb_search_key(media_type, title, year)
        current_time = int(time.time())
        
        cursor = self.conn.execute("""
            SELECT response_data, expires_at FROM tmdb_search_cache 
            WHERE cache_key = ? AND media_type = ? AND title = ? AND (year = ? OR year IS ?)
        """, (cache_key, media_type, title, year, year))
        ...
        if row:
            ...
            if current_time < expires_at:
                ...
                self.conn.execute("DELETE FROM tmdb_search_cache WHERE cache_key = ?", (cache_key,))
                self.conn.commit()
        
        return None

    def set_tmdb_search_cache(self, media_type: str, title: str, year: Optional[int], response_data: Dict[str, Any], cache_ttl_days: int = 7):
        """Store TMDb search results in cache."""
        cache_key = self._generate_tmdb_search_key(media_type, title, year)
        current_time = int(time.time())
        expires_at = current_time + (cache_ttl_days * 24 * 3600)
        
        self.conn.execute("""
            INSERT OR REPLACE INTO tmdb_search_cache 
            (cache_key, media_type, title, year, response_data, created_at, expires_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (cache_key, media_type, title, year, json.dumps(response_data), current_time, expires_at))
        self.conn.commit()

    def get_tmdb_details_cache(self, media_type: str, media_id: int, cache_ttl_days: int = 7) -> Optional[Dict[str, Any]]:
        """Retrieve cached TMDb details results."""
        cache_key = self._generate_tmdb_details_key(media_type, media_id)
        current_time = int(time.time())
        
        cursor = self.conn.execute("""
            SELECT response_data, expires_at FROM tmdb_details_cache 
            WHERE cache_key = ? AND media_type = ? AND media_id = ?
        ...
        if row:
            ...
            if current_time < expires_at:
                ...
                # Cache expired, remove it
                self.conn.execute("DELETE FROM tmdb_details_cache WHERE cache_key = ?", (cache_key,))
                self.conn.commit()
        
        return None

    def set_tmdb_details_cache(self, media_type: str, media_id: int, response_data: Dict[str, Any], cache_ttl_days: int = 7):
        """Store TMDb details results in cache."""
        cache_key = self._generate_tmdb_details_key(media_type, media_id)
        current_time = int(time.time())
        expires_at = current_time + (cache_ttl_days * 24 * 3600)
        
        self.conn.execute("""
            INSERT OR REPLACE INTO tmdb_details_cache 
            (cache_key, media_type, media_id, response_data, created_at, expires_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (cache_key, media_type, media_id, json.dumps(response_data), current_time, expires_at))
        self.conn.commit()

    def cleanup_expired_tmdb_cache(self):
        """Remove expired cache entries."""
        current_time = int(time.time())
        
        # Clean up expired search cache
        self.conn.execute("""
            DELETE FROM tmdb_search_cache WHERE expires_at < ?
        """, (current_time,))
        
        # Clean up expired details cache
        self.conn.execute("""
            DELETE FROM tmdb_details_cache WHERE expires_at < ?
        """, (current_time,))
